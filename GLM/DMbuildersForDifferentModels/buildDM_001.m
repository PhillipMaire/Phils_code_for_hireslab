function [DM, DMsettings] = buildDM_001(U, OnsetsALL_CELLS, trialTypes, cellStep)
%{
goal for this model is to include the basic variables in equal spacing and remove the shitty ones
like M0adj and FaxialAdj
...
figure;hold on 
for k = [7, 8]
% varInd = find(strcmp(DM.label, varName));
varInd = k;
plot(DM.bumpOffset(varInd)+(1:size(DM.basisFuncs{varInd}, 1)), DM.basisFuncs{varInd})
end
...
%     [DM] = shiftSigWithDM(squeeze(C.S_ctk(8, :, :)), -200:40:200, DM, 'FaxialAdj');
    ...
    %###########################
    %% set Variable
    varName = 'firstTouchOnsetTransient';
    ...
    %###########################
    %% set Variable
    varName = 'firstTouchOnsetSustained';
    firstTouch = squeeze(C.S_ctk(9, :, :));
    firstTouch(isnan(firstTouch)) = 0;
...
% % %     DM.bumpWidth
    ...
    %###########################
    %% set Variable
    varName = 'lateTouchOnsetTransient';
    ...
    %###########################
    %% set Variable
    varName = 'lateTouchOnsetSustained';
    allLateTouches = squeeze(C.S_ctk(12, :, :));
    allLateTouches(isnan(allLateTouches)) = 0;
