...
function [MeanDist, numPtsCompared,smallLineDists,  indLocFINALtoLargeLine, largeLine, smallLine] = distBet2LinesTEST (line1, line2)
...
if length(find( max(size(line1)) == size(line1))) == 1 ||length(find( max(size(line2)) == size(line2))) == 1
   error('max length of line dimensions must be larger than all other dimensions') 
end
