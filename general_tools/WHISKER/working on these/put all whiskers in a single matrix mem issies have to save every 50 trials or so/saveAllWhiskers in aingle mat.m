%%
function [everyfile] = measureFindMissedClass3
clear errorCatch
errorCatch{1} = 'blank';
...
clear allDirsToRun
counter =0;
materDirSet = {'J:\Data\Video\PHITEST\'};
...
for k = 1: length(allDirsToRun)
    ...
    d = allDirsToRun{k};
    filelist_measurements = dir([d '*.measurements']);
    allMeasureMat = nan(200000, 13,length(filelist_measurements));
    filelist_ALL{k} = filelist_measurements;
...
%         display([num2str(kk), ' out of ', num2str(length(filelist_measurements))]);
        ...
        measureMat = loadMeasurementsMat(fileNameM);
        [ysize, xsize] = size(measureMat);
        allMeasureMat(1:ysize, 1:xsize, kk) = measureMat;
    end
    everyfile{k,1} = allMeasureMat;
    everyfile{k,2} = d;
