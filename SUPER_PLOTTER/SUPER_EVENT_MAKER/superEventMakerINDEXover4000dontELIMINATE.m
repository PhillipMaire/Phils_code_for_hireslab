function               [megaPlotStruct]  =  superEventMakerINDEXover4000dontELIMINATE(U, thingToDetermineInput, varargin)
...
for cellStep = 1:length(U)
    ...
    if isstr(thingToDetermineInput)
        ...
        thingToDetermine =  squeeze(thingToDetermine);
    ...
    if isequal(size(thingToDetermine), [trialLength,numTrials])
        ...
        thingToDetermine = find(~isnan(thingToDetermine).* (thingToDetermine>0));
    end
    ...
    if nanmedian(thingToDetermineLIN)> (trialLength.*3)   % leave some wiggle room because behavior like licks for example can run over
        ...
        %get the regualr indices
        thingToDetermine2 = round((thingToDetermine2 - (floor(thingToDetermine2))).*trialLength) +1;
    ...
    if nanmedian(thingToDetermineLIN)< (trialLength.*3)  % leave some wiggle room because behavior like licks for example can run over
        ...
        % I need to remove bad stuff from things to determine before shifting becasue
        %for somereason there are things
        removedInd.OriginalLarge = find(thingToDetermine>trialLength);%&*&*
        removedInd.OriginalSmall = find(thingToDetermine<1);%&*&*
        thingToDetermine(unique([removedInd.OriginalLarge; removedInd.OriginalSmall])) = nan;
        ...
        thingToDetermine2 = thingToDetermine + shiftSigFinal;%&*&*
        
        % all events not occuring in the region we have spikes just remove them
        removedInd.startRegionLarge = find(thingToDetermine2>trialLength);%&*&*
        removedInd.startRegionSmall = find(thingToDetermine2<1);%&*&*
        %         thingToDetermine2(unique([removedInd.startRegionLarge; removedInd.startRegionSmall]))  = nan;
        
        % get the end of the signal and trim where it goes over
        endSignal = thingToDetermine2+sigL;
        ...
        BadIndsForSelectedSettings = unique([removedInd.endRegionLarge; removedInd.endRegionSmall;...
            removedInd.startRegionLarge; removedInd.startRegionSmall]);
        
        allGoodInds = intersect(find(~isnan(endSignal+makeLinInds)), find(~isnan(thingToDetermine2+makeLinInds)));
        % transform to linear inds and we're done!
        %         test21 = find(~isnan(endSignal+makeLinInds));
        %         test22 = find(~isnan(thingToDetermine2+makeLinInds));
        
        
        [~, badForSelectedSettingINDEX , ~] = intersect(allGoodInds,BadIndsForSelectedSettings );
        
        indexForGoodTrialWithTheseSettings =  ones(size(allGoodInds));
        indexForGoodTrialWithTheseSettings(badForSelectedSettingINDEX) = 0;
        
        
        
        thingToDetermineREGinds = thingToDetermine2(allGoodInds);%for normal inds
        thingToDetermine2 = thingToDetermine2 +makeLinInds;
        thingToDetermine2 = thingToDetermine2(allGoodInds);
        ...
        ultimateMat = [reshape(thingToDetermine2, size(TrialOrder)),...
            ...
            reshape(indexForGoodTrialWithTheseSettings, size(TrialOrder))...
            ];
        SS.Mat = ultimateMat;% super structs
        ultimateMatNames = {'lin-Ind start', 'lin-Ind stop','tria Index for U array', ...
            'ordered occurence of event in each trial',...
            'regular indexing start', 'regular indexing end', ...
            'index of trials with no cut offs given the current settings'};
