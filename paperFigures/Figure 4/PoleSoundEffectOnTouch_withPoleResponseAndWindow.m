...
for k = theseCells(:)'
    ...
    V2 = V{3, k};
    if V2.isSig && (length(V2.I)>5 || length(V2.E)>5)
        ...
        [~, segments, ~, ~, ~] = GET_touches(C, 'onset', false);
...
ok so delta kappa is change in curvature right? so what do I want to quantify for each touch?
 ...
 b) instantanious delta kappa, but this is and issue
        ...
        hp = squareTransparent([Twin{k}(1), Twin{k}(end)], ylim, 1, 0);% 3 to make sure it covers all data
        ...
        [spkCount, TfromPole,edges, binINDs] = binslin(Sorted1(:), tuningCurve(:), 'equalN', numBins);
        ...
        %% get pole response actual to compare with tuning curve
        sigP = V{1, k}.sig;
        
        timeFromPoleOnsetTouchOccured = segments(:, 5) - C.meta.poleOnset(segments(:, 3))';
        zeroPoint = find(V{1, k}.plotRange ==0);
        
        win3 = (timeFromPoleOnsetTouchOccured(:)+zeroPoint(:));
        win3 = (win3(:)-1) + (1:length(Twin{k}));
        
        allBLs = {};
        for kk = 1:size(win3, 1)
            allBLs{kk} = sigP(win3(kk, :), :);
        end
        
        cleanPoleWithoutPole2 = cellfun(@(x) nanmean(x(:)), allBLs)*1000
        tuningCurvePoleOnly = cellfun(@(x) nanmean(cleanPoleWithoutPole2(x)), binINDs)
        [CIset, CIsize] = cellfun(@(x) philsCIs(x ,CI_Alpha, length(x)), ...
            cellfun(@(X) cleanPoleWithoutPole2(X) , binINDs, 'UniformOutput', false),...
            'UniformOutput', false);
        CIsize = cell2mat(CIsize);
        
        
        %% plot BL pole response at touch times
        [hl, hp] = boundedline(tuningCurvePoleOnly, yTrials2plot, CIsize(:), ...
            'y-',  'orientation', 'horiz', 'transparency', 1);
        hl.Color = 'g'
        xlim([0, max(xlim)])
        xlabel('Spikes/Sec')
