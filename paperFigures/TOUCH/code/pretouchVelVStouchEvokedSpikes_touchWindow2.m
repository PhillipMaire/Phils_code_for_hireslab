...
function [] = pretouchVelVStouchEvokedSpikes_touchWindow2(U, theseCells, V, type1)
...
for k = 1:length(sortedVel)
    ...
    %     vel1 = sortedVel{k}(touchWindow{k});
    %%
    ax1 = gca;
    ...
    sig1 = sortedRastCell{k}(touchWindow{k}, :);
    sig2 = smoothTrimEdges((nanmean(sig1, 1)'), smoothSigBy);
    plot(sig2,1:length(sig2), '-k')
    
    xlim([0, max(xlim)])
    ylim(ylimTMP);% same as raster plot 
    set(gca,'YTickLabel',[]);
    set(gca,'YTick', [])
end
...
fn = '100 Touch smoothed tuning curve'
