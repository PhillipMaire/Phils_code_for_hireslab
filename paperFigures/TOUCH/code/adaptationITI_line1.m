...
% close all
function adaptationITI_line1(U, theseCells)
...
zeroNum = 0;% set to like .1 or .01 if you want to set x axis to log scale
...
%     plot([zeroNum, (betweenEdges{k}(1))],[diff1FT{k}, diff1{k}(1)] , 'o-r');
plot(xlim, [diff1FT{k},diff1FT{k}], 'r-', 'LineWidth', 2)
    h = plot(betweenEdges{k}, diff1{k}, '-ok', 'MarkerFaceColor', 'k', 'LineWidth', 2, 'MarkerSize', 4)
...
%     crush
    ...
    plot([zeroNum*ones(size(betweenEdges2(1,:))); betweenEdges2(1,:)], ...
        [diff2FT; diff2(1, :)],...
        '-', 'Color', [R1], 'LineWidth', 2)
    
    h = scatter([zeroNum*ones(size(betweenEdges2(1,:))), betweenEdges2(1,:)], [diff2FT, diff2(1, :)]);
    ...
    plot([zeroNum, betweenEdgesTMP(1)], [nanmean(diff2FT), nanmean(diff3{1})],...
        '-', 'Color', R1(1:3), 'LineWidth', 2)
    
    h = scatter([zeroNum, betweenEdgesTMP(1)], [nanmean(diff2FT), nanmean(diff3{1})]);
    h.MarkerEdgeColor = 'none';
    h.MarkerFaceColor = R1(1:3);
