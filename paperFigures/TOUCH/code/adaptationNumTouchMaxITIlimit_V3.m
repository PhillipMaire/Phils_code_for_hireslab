...
maxITItoInclude = 250;% this makes touches after 500 ms a first touch again
...
for k = 1:length(U)
    ...
    if V2.isSig
        ...
        segs = findInARowFINAL(touchOrder);
        bboys = find(interTouchInterval>maxITItoInclude);
        badBoys = bboys - segs.startInds(:)' ;
        badBoys(badBoys<0) = nan;
        [~, tmp2] = nanmin(badBoys,[],  2);
        badBoys = colonMulti(bboys, segs.endInds(tmp2));
        touchOrder(badBoys) = nan;
        interTouchInterval(badBoys) = nan;
