...
function [] = pretouchVelVStouchEvokedSpikes_P_R(U, theseCells, varargin)
...
if nargin ==3 & varargin{1} == true
    ...
    for kk = 1:2
        ...
        plot(allEdges, allPlots, 'Color', [colorCell{kk},TPval2] , 'LineWidth', 2);
        ...
        plot(betweenEdgesTMP, cellfun(@(x) nanmean(x), allPlots2), 'Color', [colorCell{kk},TPval1], 'LineWidth', 2);
        plot(betweenEdgesTMP, cellfun(@(x) nanmedian(x), allPlots2), 'Color', [colorCell{kk},TPval1], 'LineWidth', 2, 'lineStyle', '--');
    end
end
