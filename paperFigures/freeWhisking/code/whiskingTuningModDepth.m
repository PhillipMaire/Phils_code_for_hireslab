...
binnedSpikes = {};
INbins = {};
numIterationsForBootStrap = 1000;
varName = {};
binIndex = {};
fitCOSINE = true;
Whisk = struct;
...
for varTypeTMP = 1:length(theseVars)
    ...
    for cellStepTMP = 1:length(theseCells)
        ...
        var1(allMask) = nan;
        [spikes, var1,edges]=binslin(var1(:), spikes(:), 'equalN', numBins);
        var1= cell2mat(var1);
...
MIwhisking = MIwhisking';
varNames = {'Theta' 'Velocity' 'amp' 'setpoint' 'Phase' 'maxDelta Kappa' };
% MI_mat = reshape(cell2mat(MI), sum(cellfun(@(x) ~isempty(x), MI)), []);

[~, x1] = find(ones(size(MIwhisking)));
figure
MIwhisking(isnan(MIwhisking)) = 0;
beeswarm(x1, MIwhisking(:),'sort_style','hex','overlay_style','sd')
xticks(1:length(varNames))
xticklabels(varNames)
ylim([0 1])
figure
x1 = x1(MIwhisking<.99);
MIwhisking = MIwhisking(MIwhisking<.99);
