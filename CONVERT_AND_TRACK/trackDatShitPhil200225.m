function trackDatShitPhil200225(varargin)
...
% ####################### make this a user variable l
global dirsToSeachForAVIshells
dirsToSeachForAVIshells{end+1} = searchInAllOfTheseDirs;% add the OG search directory incase there are avi shells in there;
dirsToSeachForAVIshells{end+1} = sprintf('%s\\Downloads%sAVI_SHELLS', getenv('USERPROFILE'), filesep); % add the default directory for avi shell files
dirsToSeachForAVIshells = unique(dirsToSeachForAVIshells);
...
if exist('MParallel.exe','file') == 2
    ...
    try
        ...
        error('Cannot find MParallel.exe on the path. please add to matlab path')
    end
end
...
% % %             %#### edit this I dont want to make this a thing, just make a
% % %             %test for corruption or just delete the most recent one or make
% % %             %it delete when  it fails as a catch
...
% % %             end
            ...
            if convertVideoReally == 1
                ...
                %% *****************************
                %% *****************************
                copyAVIsAndConvertToMP4(startDir, numCores, videoQuality)
                %% *****************************
                %% *****************************
        ...
        %% to do
        % automate the filling in and creating of exe and system mparralell command
        %
        % make sure to have a condition if there is no avi properly name it, place
        % it in the default directory and then copy that one and make as many
        % properly names files as possible
...
%%%%%%%%%% program 2 start
    ...
    function [] = norpix_seq_reader_jsy_NESTED321865468546(seqList, startDir, numCores)
...
%%%%function5 start
    ...
    function aviShellList = getAVIshellDIrs32945398(dirsToSeachForAVIshells)
        
        aviShellList = {};
        for numDirs = 1:length(dirsToSeachForAVIshells)
            AVIdirs = regexp(genpath(dirsToSeachForAVIshells{numDirs}),['[^;]*'],'match');
            for k_avi = 1:length(AVIdirs)
                aviShellTMP = struct2cell(dir([AVIdirs{k_avi}, filesep, 'AVIshell_*']));
                for AVIlist = 1:size(aviShellTMP, 2)
                    AVInameTMP = [aviShellTMP{2, AVIlist}, filesep, aviShellTMP{1, AVIlist}];
                    aviShellList{end+1, 1} = AVInameTMP;
                    str1 = (strfind(AVInameTMP, 'AVIshell_')+9);
                    dashLoc = strfind(AVInameTMP, '_');
                    dashLoc = dashLoc(dashLoc>str1);
                    end1 = dashLoc(2)-1;
                    aviShellList{end, 2} = AVInameTMP(str1:end1);
                    aviShellList{end, 3} = str2num(AVInameTMP(str1+1:dashLoc(1)-1));
                    aviShellList{end, 4} = str2num(AVInameTMP(dashLoc(1)+2:dashLoc(2)-1));
                    aviShellList{end, 5} =  aviShellTMP{1, AVIlist};
                    aviShellList{end, 6} =aviShellTMP{2, AVIlist};
                    aviShellList{end, 7} = AVInameTMP(str1-9 : dashLoc(end));
                    aviShellList{end, 8} = str2num(AVInameTMP(dashLoc(end)+1 : end-4));
                end
            end
        end
        %
        [~,remDubs,~] = unique(aviShellList(:, 1));
        aviShellList =  aviShellList(remDubs, :);
    end




    function [ShellFolder, aviShellList2] = CopyOverNeededAVIshells51053453(aviShellList, iHeight, iWidth, numCores)
        % this only aplies if there are less avi shells than the number of
        % cores user chose to use
        %% check for good avi shell files
        % make a folder in the users download folder as the default avi shell
        % folder to add new files to
        ShellFolder = sprintf('%s\\Downloads%sAVI_SHELLS', getenv('USERPROFILE'), filesep);
        mkdir(ShellFolder);
        
        % get only the shell avis that are good for this video
        aviShellList2 = aviShellList(find((cell2mat(aviShellList(:, 3))==iHeight).*(cell2mat(aviShellList(:, 4))==iWidth)), :);
        %% copy shell avis if we need more for more cores, have a standard folder for this
        if size(aviShellList2, 1) ~= 0 % if there are no files we need to create one and cpy them
            goodNums = setdiff(1:10000, cell2mat(aviShellList2(:, 8)));% a bunch of free numbers for naming
            if numCores>size(aviShellList2, 1)
                disp(['We have ', num2str(size(aviShellList2, 1)), ' AVI shells, copying over ',...
                    num2str((numCores-size(aviShellList2, 1))), ' more. Please wait...']);
                for CopyShells = 1:(numCores-size(aviShellList2, 1))
                    try
                        copiedShellName = [ShellFolder, filesep,aviShellList2{1, 7}, num2str(goodNums(CopyShells)), '.avi'];
                        copyfile(aviShellList2{1, 1}, copiedShellName)
                        aviShellList2{end+1, 1} = copiedShellName;
                    catch ME
                        if ~contains(ME.message, 'Cannot copy or move a file or directory onto itself.')
                            rethrow(ME)
                        end
                    end
                end
            else
                disp('Great! We have enough AVI shells let''s proceed')
                
            end
            
        else % make
            disp('There are no compatible AVI shells. Creating one...')
            return
        end
    ...
    function [] = copyAVIsAndConvertToMP4(startDir, numCores, videoQuality)
        
        
        seqList = dir([startDir, filesep, '*.seq']);
        modeSize = mode(cellfun(@(x) x(:) , {seqList.bytes}));% get mode of vid size for this folder
        goodSizeIndex = cell2mat({seqList.bytes}) == modeSize; % keep only videos of that size
        if sum(~goodSizeIndex)>0 % warning about not converting different sized SEQ files
            badBoys = find((~goodSizeIndex));
            tmp1 = cellfun(@(x) [x, '\n'], {seqList(badBoys).name}, 'UniformOutput', false);
            warning(sprintf(['\nNot converting the following SEQ files because they are the wrong size \n'...
                , strcat(tmp1{:})]));
        end
        seqList = seqList(goodSizeIndex);
        
        seqIn = [seqList(1).folder, filesep, seqList(1).name];
        ...
        iTrueSize = fread(seqID, [1], 'ulong');
        %################
        numFrames = (modeSize - 8192)./iTrueSize;
        readSize = iWidth*iHeight;
        frameIndex = 0:numFrames-1;
        startAVI = 65832+(frameIndex*(readSize+8));
        startSEQ = 8192+(frameIndex*(readSize+7168));
        %################
        %% this will be used at each folder to find the correct number of avi shells and
        % then it will be used to create EXE files to replace the video in them
        % then they will be converted to mp4, following that they will converted to
        % MP4 then they will be used again in the same way simply replacing the
        % images but keeping the shells!!!!!!!!!
        % 1) get all avi shell files
        aviShellList = getAVIshellDIrs32945398(dirsToSeachForAVIshells);
        % 2) make a new variable with only the correct sized avi files AND copy
        % over and more needed to match the number of cores being used (
        [locAVIfolder, aviShellList2] = CopyOverNeededAVIshells51053453(aviShellList, iHeight, iWidth, numCores);
        if size(aviShellList2, 1) == 0  %if there where no videos make one and then copy it
            tmpDir = pwd;
            cd(locAVIfolder)
            makeOneAVIshell2938472390(seqList)
            aviShellList = getAVIshellDIrs32945398(locAVIfolder);
            [locAVIfolder, aviShellList2] = CopyOverNeededAVIshells51053453(aviShellList, iHeight, iWidth);
            cd(tmpDir)
        end
        
        
        %% now we have enough shell avis to use all the cores requested
        rem1 = rem(size(seqList, 1), numCores);
        base1 = floor(size(seqList, 1)./ numCores);
        numClusts = nansum([base1, rem1./rem1]);
        frameAndLoc = setFrameSizeAndLocation(readSize, startSEQ, startAVI, numFrames);
        exeName = [seqList(1).folder, filesep, 'insertSEQintoAVIshell'];
        %         tmp1 = struct2cell(seqList)';
        
        delete([exeName, '*.exe']);
        delete([exeName, '*.c']);
        tic
        for clusters1 = 1:numClusts % numCOres
            if clusters1~=numClusts || rem1==0
                ind1 = ((clusters1-1).*numCores) + (1:numCores) ;
            else
                ind1 = ((clusters1-1).*numCores) + (1:rem1) ;
            end
            % make a variable just for running this
            for coreNums = 1:length(ind1) %groups of seq files equal to the number of cores requested
                makeEXEforpopulatingAVIshell(seqList, ind1(coreNums),aviShellList2, coreNums, frameAndLoc);
            end
            % this is actaully faster with not in parallel so set to 1
            exeCMDpar = sprintf('dir /b *.exe | %s --count=%.00f --shell --stdin --pattern="{{0}}"', mpPath, numCores);
            system(exeCMDpar)
            ...
            fullFileName = [];
        for fullNamek = 1:size(seqList, 1)
%             fullFileName = [fullFileName, ' "', aviShellList2{fullNamek, 1}, '" , "', seqList(fullNamek).folder, filesep, seqList(fullNamek).name, '" :'];
 fullFileName = [fullFileName, ' ffmpeg -y -i "', aviShellList2{fullNamek, 1}, '" -c:v mpeg4 -qscale:v ',num2str(videoQuality),...
                ' -c:a libmp3lame -qscale:a 1 -vf "transpose=0,transpose=2" "', seqList(fullNamek).folder, filesep, seqList(fullNamek).name, '" :']
        end
       fullFileName = strrep(fullFileName, '.seq', '.mp4');
            
            %% where we run ffmpeg 
            system(['"', mpPath, '" --count=',num2str(numCores), fullFileName(1:end-2)])
    ...
    function [tmp10] = setFrameSizeAndLocation(readSize, startSEQ, startAVI, numFrames);
        
        
        tmp10 = [...
            '	size = ' num2str(readSize) ';	\n	'...
            '	memblock = new char [size];	\n	'...
            ];
        for knFrames = 1:numFrames
            tmp8 = [ ...
                '	modFile.seekg (' num2str(startSEQ(knFrames)) ', ios::beg);	\n	'...
                '	modFile.read (memblock, size);	\n	'...
                '	infile.seekp (' num2str(startAVI(knFrames)) ', ios::beg);	\n	'...
                '	infile.write(memblock, size);	\n\n	'...
                ];
            tmp10 =   [tmp10,  tmp8];
    ...
    function  makeEXEforpopulatingAVIshell...
            (seqList, k_seq,aviShellList2, coreNums, frameAndLoc)
        
        
        disp([num2str(k_seq), '/', num2str(length(seqList))]);
        
        copy_SEQ = [seqList(k_seq).folder, filesep,  seqList(k_seq).name];
        [copy_AVI_DIR,~,~] = fileparts(copy_SEQ);
        
        shellAVI = aviShellList2{coreNums, 1};
        shellAVI(strfind(shellAVI, '\\')) = '';
        shellAVI = strrep(shellAVI, filesep, repmat(filesep, 1, 4));
        
        copy_SEQ(strfind(copy_SEQ, '\\')) = '';
        copy_SEQ = strrep(copy_SEQ, filesep, repmat(filesep, 1, 4));
        
        
        
        
        
        % delete('C:\Users\maire\Desktop\trstSEQ\myFileCompressed2.avi')
        % tic
        %         copyfile('C:\Users\maire\Desktop\trstSEQ\myFileCompressed2 - Copy.avi', 'C:\Users\maire\Desktop\trstSEQ\myFileCompressed2.avi')
        fname = 'insertSEQintoAVIshell';
        fidC = fopen([fname, '.c'],'w');
        CplusProg= [...
            '	#include <iostream>	\n	'...
            '	#include <iomanip>	\n	'...
            '	#include <fstream>	\n	'...
            ...
            '		\n	'...
            '	ifstream modFile ("',copy_SEQ ,  '", ifstream::binary);	\n	'...
        ...
        fprintf(fidC, CplusProg);
        fclose('all');
        exeName = [copy_AVI_DIR, filesep, fname];
        exeName(strfind(exeName, '\\')) = '';
        exeName = strrep(exeName, filesep, repmat(filesep, 1, 2));
        cmdtoRun = ['g++ -o "', exeName , '_',num2str(k_seq) , '.exe" "', exeName , '.c" '];
        system(cmdtoRun);
    end
