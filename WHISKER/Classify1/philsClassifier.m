...
function [] = philsClassifier(startDir, Lthresh)
trackingINfoSaveDir =  'X:\Video\PHILLIP';
...
allDistances = [];
maxDistFromAveFollice = 25;

% BOTH CAN BE FALSE THOUGH
if all([pickClosestToPole,  pickClosestToFolicleIfWlengthsAreSimilar])
error('user must pick either closest to pole of closest to folicle or neither but not both' )
end
%%
        ...
        if all(test1) && length(measF)>0
            ...
            while 1
                ...
                for kk = 1:length(tmpU2)
                    ...
                    if isempty(find((measurmentMat2(:, 1) == tmpU2(kk)) .* (measurmentMat2(:, 3) == wskrNUM))) && ONLY_RECLASSIFY_EMPTY_FRAMES%identify frames with no whisker tracked
                        ...
                        if length(theseOnes)>1
                            ...
                            distMin = [];IDnumsW = [];distdistMinFolMin = [];
                            allLengths = [];
                            ...
                            elseif pickClosestToFolicleIfWlengthsAreSimilar
                               allDistances(end+1:end+length(distdistMinFolMin)) = distdistMinFolMin;
                                [minDist, closestW] = (min(distdistMinFolMin));
                                closestW = theseOnes(closestW); % classified whisker ind
                                
                                if maxDistFromAveFollice > minDist
