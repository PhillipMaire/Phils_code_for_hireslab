...
UserInputThreshold = 40; % this variabe sets another variable below
...
materDirSet = {'Z:\Data\Video\PHILLIP\AH0706\171101'};
    ...
    for kkk =1:2 %smaller and larger files
        ...
        for kk = 1: numel(dAll)
            ...
            if length(filelist_mp4_mat) < thresholdNum4000Vids  &&  length(filelist_mp4_mat)>1
                thresholdNum4000Vids =  length(filelist_mp4_cell);
                display(['          autoset threshold length to ', num2str(thresholdNum4000Vids), ' because so few videos...']);
            end
            ...
            if ~isempty(filelist_mp4_cell)
                ...
                if largestBadVid>0 && counterOf4000 >= thresholdNum4000Vids
                    ...
                    slashThirdFromBack = slashLocations(end-2);
                    newD = [d(1:slashThirdFromBack),folderString,d(slashThirdFromBack+1:end)];
                    mkdir(newD);
                    ...
                    if numSuspiciousVids > 0
                        ...
                        if sepSuspiciousFilesOut ==1 %%% only run is user wants
                            newD = [d(1:slashThirdFromBack),'suspiciouslySmall4000frames\',d(slashThirdFromBack+1:end)];
                            mkdir(newD);
