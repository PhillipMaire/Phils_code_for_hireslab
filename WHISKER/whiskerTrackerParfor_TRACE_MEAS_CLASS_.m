...
mainDir = 'Z:\Data\Video\PHILLIP\classCorrectlyTest';
...
copyOwnDefaultParames = 0; %if you copy your own default params by hand then set to 1
trace = 1 ;
measure = 1;
classify = 1;
reclassify = 0;
    ...
    if status
        ...
        if classify == 1
            ...
            parfor n=1:length(classes)%################parfor
                ...
                commandString = ['classify ' classes(n).name ' ' outputFileName '.measurements ' 'top ' '--px2mm ' '0.033 ' '-n ' '-1 ' '--limit5.0:15.0 ' ]
                system(commandString);
