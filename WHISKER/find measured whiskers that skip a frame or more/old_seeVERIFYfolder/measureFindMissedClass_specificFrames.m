%%
function measureFindMissedClass_specificFrames
...
materDirSet = {'Z:\Data\Video\PHILLIP', 'Y:\Whiskernas\Data\Video\PHILLIP', 'D:\backup\Data\Video\PHILLIP'};
...
for k = 1: length(allDirsToRun)
    ...
    for kk = 1:length(filelist_measurements)
        ...
        fprintf( fid, 'directory %s \nall Whiskers Red %s \nmost whisker red %s \nsome missed frames %s\n\n\n####next\n\n\n', ...
            allDirsToRun{k}, num2str(allRed), num2str(mostRed), num2str(someMissed));
...
%%
    ...
    function [missingFrames, indexDupFrames, dupFrames, atLeastOneFrameTracked, DubFrameNum_and_Count] = doubleAndMissing(whiskerIndex)
        ...
        dupFrames = dupFrames(DubFrameNum_and_Count(:,2)>1);
