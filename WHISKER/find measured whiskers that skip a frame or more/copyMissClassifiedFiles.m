function copyMissClassifiedFiles(finalList)

testMadeDir = 'start';
counter = 0;
for k = 1: length(finalList)
    findPhilllip = strfind(finalList{k,4},'PHILLIP');
    newDir = finalList{k,4};
    newDir((findPhilllip+3):(findPhilllip+6)) = 'TEST';
    
    if isempty(strfind(testMadeDir, newDir))
       mkdir(newDir)
       counter = counter +1;
       testMadeDir(end+1:end+length(newDir)) = newDir;
    end
    copyfile(finalList{k,3},newDir);
    copyfile(finalList{k,6},newDir);
    copyfile(finalList{k,7},newDir);
    progressbarNESTED(k/length(finalList))
end
