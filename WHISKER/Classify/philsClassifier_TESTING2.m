...
startDir = 'C:\testingClass\';
 philsClassifier_TESTING2(startDir)
toc

%}
%% jus to save info, should always be the same
function [] = philsClassifier_TESTING2()
startDir = 'C:\testingClass\';
...
Lthresh = 90; %length threshold. whisker must be larger than this
% threshMISSforceLargest = 1; % automatically classify the largest whiskeras %***********************
% centerOfMassJumpPrevention = 1%***********************
maxCenterOfMassChange = 25;% mean distance of x and y of previous whisker min distance to all points of testing whisker
...
% % % % %             [~, selectedIndexW] = ismember(toIndexW,IDandTIME_W,'rows');
            ...
            toIndexW = [measurmentMat(:,2),measurmentMat(:,1)];
            [~, selectedIndexW] = ismember(toIndexW,IDandTIME_W,'rows');
...
%                         keyboard
%                     end
                    if isempty(find((measurmentMat2(:, 1) == tmpU2(kk)) .* (measurmentMat2(:, 3) == wskrNUM)))%identify frames with no whisker tracked
                        if tmpU2(kk) == 778
                            keyboard
                        end
...
%                             timeBefore = ([whiskerstruct.time] <= tmpU2(kk)-1);
                            
                            
%                             measTMP = [measurmentstruct.label; measurmentstruct.fid]';
%                             correctLabel = (measTMP(:, 1) == wskrNUM);
%                             timeBefore = (measTMP(:, 2) <= tmpU2(kk)-1);
%                             Wid = [measurmentstruct.wid];
%                             correctLabel2 = Wid(find(correctLabel));
%                             
%                             %
                                                        correctLabel = (IDandTIME_W(:, 1) == wskrNUM);
                                                        timeBefore = (IDandTIME_W(:,2) < tmpU2(kk));
...
%                             measTMP(lastWhiskerInd, :)
                            whisker2lookat = whiskerstruct(lastWhiskerInd);
                            ...
                            crush; 
                            figure; 
                            plot(tmpwWbefore.x, tmpwWbefore.y, 'r-')
                            hold on 
                            plot(tmpWbeforeCenterOfMass(1), tmpWbeforeCenterOfMass(2), 'ro')
                            ...
                            %                             [~, sortedIndsBar ] = sort( distMinBar(possibleINds));
                            %
                            
                            [tmp1 , tmp2 ] = min(distMinLastW)
                            for kkk = 1:length(theseOnes) %replace the clasification in the struct to save later GOOD JOBO
                                ...
                                IDandTIME_W(theseOnes(kkk), 1) = -1;
                            end
                            if ~isempty(tmp2)
                                closestW = tmp2
                                closestW = theseOnes(closestW); % classified whisker ind
%                                  measurmentstruct(theseOnes)
                                measurmentstruct(selectedIndexW(closestW)).label = 0; % replace just the one you want with the correct whisker number
                                
                                IDandTIME_W(closestW, 1) = 0;
