...
philsClassifier_3('C:\Users\maire\Desktop\trstSEQ\New_folder\New folder (2)', 100)
%}
%% jus to save info, should always be the same
function [] = philsClassifier_3(startDir, Lthresh)
trackingINfoSaveDir =  'C:\Users\maire\Downloads\tmpdir';
dateString1 = datestr(now,'yymmdd_HHMM');
%% get all the files from every dir
ifOnlyOneIdentifiedTraceInAsingleFrameSetAsMainWhisker = 0;% this is dangerous if you for some reason only have a random trace and not your main trace or any other traces
% highly suggest keep this to false
...
allDistances = [];
maxDistFromMostRecentFollice = 15; %if more than this then it is not the correct whisker
                    ...
                    if isempty(find((measurmentMat2(:, 1) == tmpU2(kk)) .* (measurmentMat2(:, 3) == wskrNUM))) && ONLY_RECLASSIFY_EMPTY_FRAMES
                        ...
                        if length(theseOnes)>1
                            ...
                            for kkk = 1:length(theseOnes)
                                ...
                                IDnumsW(kkk) = find(all(IDandTIME_W == IDnumsM, 2));% find this whisker trace in the whikser file ( indexes are different in whisker and meas files)
                                tmpW = whiskerstruct(IDnumsW(kkk));
                                
                                tmpM = measurmentMat2(theseOnes(kkk), :);
                                
                                allLengths(kkk) = tmpM(6);
                            ...
                            for kkk = 1:length(theseOnes) %replace the clasification in the struct to save later GOOD JOBO
                                ...
                                measurmentMat(theseOnes(kkk), 3) = -1;
                            end
                            if pickClosestToPole == true
                                ...
                                measurmentMat(closestW, 3) = 0;
                            elseif pickClosestToFolicleIfWlengthsAreSimilar
                                allDistances(end+1:end+length(distMinFolMinNearest)) = distMinFolMinNearest;
                                if sum( distMinFolMinNearest<maxDistFromMostRecentFollice)>1%2 or more close to teh follice (or tip) take the longest of those that are close enough to the whikser follice
                                    allLengths2 = allLengths;
                                    allLengths2(~(distMinFolMinNearest<maxDistFromMostRecentFollice)) = nan;
                                    [~, bestW] = nanmax(allLengths2);
                                    bestW = theseOnes(bestW);
                                    measurmentstruct(bestW).label = int32(0);
                                    measurmentMat(bestW, 3) = 0;
                                elseif sum( distMinFolMinNearest<maxDistFromMostRecentFollice) == 1 % if only 1 meets the criteria use it
                                    [~, bestW] = (min(distMinFolMinNearest));
                                    bestW = theseOnes(bestW); % classified whisker ind
                                    measurmentstruct(bestW).label = int32(0);
                                    measurmentMat(bestW, 3) = 0;
                                    
                                end
                            else % i think this is simply just choose the largest whisker
                                ...
                                measurmentMat(largestW, 3) = 0;
                                
                            end
                        else % just set the largest whisker
                            if ifOnlyOneIdentifiedTraceInAsingleFrameSetAsMainWhisker
                                measurmentstruct(theseOnes).label = int32(0);
                                measurmentMat(theseOnes, 3) = 0;
                            end
                        end
                    end
                end
