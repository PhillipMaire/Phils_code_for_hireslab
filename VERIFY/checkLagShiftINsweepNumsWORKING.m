%% for if there was a shift in the middle of the trial
...
fid = fopen( [textFileLoc, filesep, 'islagShiftCorrect SWEEPNUMS ', dateString, '.txt'], 'wt' );
%% load cell building var


catchcell = cell(0);
for k = 1:length(cellNumberForProjectString)
    disp(num2str(k*100/length(cellNumberForProjectString)))
    try
        ...
        for i = 1:length(dirTrialNums)
            ...
            is_i_a_bTrial = find(BehaviorTrialNums == i,1); %index for trial i in the behavior files if it exists
            if ~isempty(is_i_a_bTrial) %if iter variable i matches the behavior trial number
                is_bTrial_a_sweepnum = find(sweepnums == BehaviorTrialNums(is_i_a_bTrial));%index for trial i in sweep nums (just used for below if statement
                if ~isempty(is_bTrial_a_sweepnum)
                    tmp2 = b.trials{is_i_a_bTrial}.motorPosition; %motor position for trial
                    motor2(BehaviorTrialNums(is_i_a_bTrial)) = tmp2;
                end
            end

%             dirTrialNums are the numbers derived from filelist (measurment files) 
            if ~isempty(find(dirTrialNums == i,1)) % basically if measure file with this number, i, exists
...
% % % % % %         barxmotor2Trimmed = barxmotor2(sum(isnan(barxmotor2),2) == 0, :)
% % % % % %         [sortedA indexA] = sort(barxmotor2Trimmed(:,1));
% % % % % %         [sortedB indexB] = sort(barxmotor2Trimmed(:,2));
% % % % % %         for kkkk = 1:length(indexA)
% % % % % %         refSorted(kkkk, 1) = find(indexA(kkkk) == indexB);
% % % % % %        
% % % % % %         end
% % % % % %         
        ...
        hold on
        % plot(bv,'go')
        resultsString = ['Behavior Lag = ' num2str(lag_shiftBV) '. Motor Lag = ' num2str(lag_shiftMOTOR) '. MotorLag2 = ' num2str(lag_shiftMOTOR2), 'MoterDiff ',num2str(lag_shiftMOTOR2-lag_shiftMOTOR) ]
