...
function [indexingMat,spksToPlotGood,spksToPlotBad, good, bad]= ...
    lick_sorting_PSM(TrialIndCount,swall,scaleORaligned, preSpikeBaselineTime,varargin)
if nargin ==4
    %creat indexingMat variable if not passed through
    indexingMat = ones(length(TrialIndCount), 10*3);%1 mean greater than 0 means less than
...
elseif strcmp(lower(scaleORaligned), 'aligned')
    swAdjusted = swcat - repmat(sum(swcat(preSpikeBaselineTime,:))/numel(preSpikeBaselineTime),30,1);
    plotLimY = [-10 25];
else
...
for addNum= startNum: length(TrialIndCount)%%%%adjusted this to work with one trial at a time 
    allSpikes =spikeCountPerTrial(1:TrialIndCount(addNum));
    spikeCountPerTrialChunk = spikeCountPerTrial(TrialIndCount(addNum));
    if sum(spikeCountPerTrialChunk)<1 %dont use no spike trials
        delete(spikeHandle)
        FigNameString = ['Plot ',num2str(addNum),'/', num2str(length(TrialIndCount)),'     Trials ', num2str(TrialIndCount(addNum)),'     Spikes ',num2str(test3), '-' num2str(test1)];
        title(FigNameString)
        lineMat(1:31) = -1;
        line(0:30, lineMat);
...
%     spikeCountPerTrialChunk = spikeCountPerTrial(TrialIndCount(addNum):TrialIndCount(1+addNum)-1);
    ...
    for i = 1:numel(swallChunk) %built to filter out each trial individually so can cross validate w/ licks
        ...
        spksToPlotBad{INDEXnum} = [];
            ...
            elseif strcmp(lower(scaleORaligned), 'aligned')
                %use aligned spikes
                swSingle = swcatSingle - repmat(sum(swcatSingle(preSpikeBaselineTime,:))/numel(preSpikeBaselineTime),30,1);
            end
            ...
            if ~isempty(swallChunk{i});%%%%%%%%%%%%%%%%%%%%
                for k = 1:size(indexingMat, 2)/3
                    ...
                    elseif  indexingMat(addNum,  (k-1)*3 + 2) == 0%less than case
                        badAligned{INDEXnum,k} = find(swSingle(indexingMat(addNum,  (k-1)*3 + 1),:) < indexingMat(addNum,  (k-1)*3 + 3));
                    else
