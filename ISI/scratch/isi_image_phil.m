function isi_image_phil(filename)
...
% raw data:
newMat = (0:20:380);
timeMat = zeros(size(read_qcamraw([filename '.qcamraw'], (newMat+1))));
if exist([filename '.qcamraw'],'file')
    f = 1;
    for k = 1:nchunks 
        rep = read_qcamraw([filename '.qcamraw'], (newMat+k));

        timeMat(:,:,k) = mean(rep(:,:,:),3)/nchunks;%dont need to divide her do I?
    ...
    diffMean = 
    % Didn't find .mat file, so wri(stimMean - baseMean)./baseMean * 100;te it now:
