function [imageVar] = isi_video3(filename,frameRate, varargin)
...
% optimal setting is contrast at 2 and brightness and gamma at their defaults
...
% % % % % % smoothingStepSize = 60; % whisker stimulation total 5 sec (at 5 Hz)%%###
% % % % % % smoothingFramesPerFrame = 10;


% %
%%%% setting for normal ISI settings
% % % % % % % % % contrastAdjLims = [0.4 0.6];
...
elseif nargin == 3
    disp('using contrast adjustment with FFMPEG');
    mpegContrast = true;
elseif nargin>3
...
totalFrames = length(find(framesIndex2<=20));
...
for movieIter = 1:totalFrames
    ...
    disp(['frames ''', num2str(stimPeriod), ''' of 20 ',...
        '  to make frame ', num2str(movieIter)]);
