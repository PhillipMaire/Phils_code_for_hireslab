...
trialsCruv = ceil(allTouches./4000);
     
       imagesc(curveTouch)
       %%
       
[sorted, index]=       sort(mean(curveTouch(:, 102:104), 2))
%%
 figure
         curve = squeeze(cellTMP.S_ctk(6,:,:));
       curveTouch = curve(allTouches+(-100:100));
      ...
      curveTouch2 = curveTouch(index, :);
       ...
       for testk = 1:size(curveTouch, 1)
       plot(1:size(curveTouch, 2), curveTouch(testk,:)+testk*ones(1,size(curveTouch, 2)))
       hold on 
       end
    %%
    onAndOff = [allTouches(:), allTouchesOFF(:)];
    for testk = 1;size(onAndOff, 1)
        
        allTouches(k)-10: allTouches(k) +10
