...
for cellStep = 1:length(U)
    ...
    %     allTouches = ([touchFirst(:);allOtherTouches(:) ]);
    
    if plotAllTouches == 1
        [segmentsTMP] = findInARow(touchFirst);
        touchFirst= segmentsTMP(:,1);
        [segmentsTMP] = findInARow(allOtherTouches);
        allOtherTouches= segmentsTMP(:,1);
    ...
    for k = 1 :length(inds1)
        ...
        if ~isempty(inds1{k})
            for kk = 1:length(inds1{k})
                tmpSpikes = spikes(:,trialNums(inds1{k}(kk)) );
                %                 tmp8(end+1) = inds1{k}(kk);
                win1 = firstTouchTime(inds1{k}(kk))+ poleOn(trialNums(inds1{k}(kk)))+rangeOfSig;
                win1(win1>4000) = 1;
                win1(win1<1) = 1;
                tmp1 = tmpSpikes(win1);
                tmp1(win1>4000) = nan;
            ...
            toPlot = smooth(nanmean(tmpAllSpikes, 2), 10);
