...
for cellStep = 1:length(U)
    ...
    if singleMeanPOLE>= spkForStimThresh
        ...
        if ~isempty(find(ZSCOREDmeanWHISKspikes(onlyPlotTheseInds)>Zthreshold))
            tmpPLOT = plot(signalTIME(onlyPlotTheseInds), ZSCOREDmeanWHISKspikes(onlyPlotTheseInds));
        ...
        figure(100);
        hold on
        if ~isempty(find(ZSCOREDmeanPOLEspikes(onlyPlotTheseInds)>Zthreshold))
            tmpPLOT = plot(signalTIME(onlyPlotTheseInds), ZSCOREDmeanPOLEspikes(onlyPlotTheseInds));
            tmpPLOT.Color = [tmpPLOT.Color 0.5];
            ylim(ylimSET);
            xlim(xlimSET);
            %     axis tight
...
keyboard
set(gcf, 'Units', 'Normalized', 'OuterPosition', [1, 0, 1, 1]);
filename =  [num2str(cellStep), 'W_aligned ZSCORE smooth thresh_2 ', num2str(smoothFactor) ' cropped' ];
