...
for varNumber = 1:6
    ...
    for cellStep = 1:length(U)
        ...
        normPREspks = SPKperBinPRE./(normalizeByPRE+0.0000001);
        normPREspks = normPREspks.*1000;%spk per sec
        normPREspks = normPREspks(:);
        TMP1 = boundedline(binsForVar2, normPREspks,normPREspks- CIpre(:,1),'-bo', ...
            'alpha','transparency', transp);
        
        hold on
        normPOSTspks = SPKperBinPOST./(normalizeByPOST+0.0000001);
        normPOSTspks = normPOSTspks.*1000; % spks per sec
        normPOSTspks = normPOSTspks(:);
        [l,p] = boundedline(binsForVar2, normPOSTspks,normPOSTspks- CIpost(:,1),'-ro', ...
            'alpha','transparency', transp);
        ...
        %          TMP3 =  boundedline(binsForVar2, normPREspks,normPREspks- CIpre(:,1),'-bo', binsForVar2, normPOSTspks,normPOSTspks- CIpost(:,1),'-ro');   %,...
        %               'alpha', alphaSET,'transparency', transp);
        %%
        %         boundedline(x, y, b)
        %%
        if false
        ...
        saveLoc = ['C:\Users\maire\Documents\PLOTS\pole_up_ModulationLINE_CI_TRANSPARENT\' varName '\'];
        mkdir(saveLoc)
