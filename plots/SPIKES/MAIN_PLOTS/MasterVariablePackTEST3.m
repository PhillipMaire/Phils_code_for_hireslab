...
for TYP = 1:size(masterMat, 2)
    for CS = 1:size(masterMat, 1)
        if masterMat(CS, TYP, 32) % if cell was found to be significant
            ...
            percMax = 0.95;
            Ssig =(Ssig-nanmin(Ssig))./nanmax(Ssig-nanmin(Ssig));
            
            if UPorDOWN(CS, TYP) %positive
                PKtime = find(Ssig>=percMax, 1);
...
for k = 1:3% these are the 3 unique variables touch cue sound and whisking onsets
    ...
    Xtxt = -30;
    tmp = nan(401, 15);
    tmp(1:401, 1:size(allRespPos, 2)) = allRespPos;
    allRespPos = tmp;
...
xticks = linspace(1, 401, numel(xticklabels));
set(gca, 'XTick', xticks, 'XTickLabel', xticklabels)
    
    
    
    tmp = nan(401, 15);
    tmp(1:401, 1:size(allRespNeg, 2)) = allRespNeg;
    allRespNeg = tmp;
