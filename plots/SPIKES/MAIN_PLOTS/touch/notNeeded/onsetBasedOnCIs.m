...
for cellStep = 1:length(U)
    ...
    tmpRET = allTouchRet{cellStep};
    
    
    tmpRET_BL = tmpRET(:, 1:trialTimeToMatch-1);
    tmpRET_BL = nanmean(tmpRET_BL, 1);
   tmpRET_BL = tmpRET_BL(~isnan(tmpRET_BL));
     [CI_ret sizeCI_ret] = philsCIs(tmpRET_BL ,aphaForCI);
    ...
    xlimTMP = xlim;
    plot(xlimTMP, [CI_ret(1) CI_ret(1)], 'k-');
    plot(xlimTMP, [CI_ret(2) CI_ret(2)], 'k-');
%    keyboard
