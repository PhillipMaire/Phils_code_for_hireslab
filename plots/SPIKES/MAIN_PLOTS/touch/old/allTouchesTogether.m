...
%     imagesc((allTouchesHeat))

% for plotIter = 1:size(allTouchesHeat,1)
figure
xToPlot = repmat(1:size(allTouchesHeat, 2), [size(allTouchesHeat, 1), 1]);
for  plotThese = 1:size(allTouchesHeat, 1)
    tmp = plot(xToPlot(plotThese, :), allTouchesHeat(plotThese, :), '-k');
    hold on
    tmp.Color = [tmp.Color, 0.01];
end
%     tmp = plot(xToPlot', allTouchesHeat', 'k-');
%     tmp.Color = [tmp.Color, 0.05];
%     set(tmp, {'alpha'}, {[tmp.Color, 0.05]})
% end
