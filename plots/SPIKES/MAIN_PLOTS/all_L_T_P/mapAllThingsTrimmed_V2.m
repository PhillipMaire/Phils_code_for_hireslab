%% build everything based on



% %% this is so that the number of iterations match the neurons we have  match below that is all
% xyxColorEVAL = ['xyxColor = Var2.' varName3{k} '.allTraces;'];
% eval(xyxColorEVAL);
% ColMapEVAL = ['ColMap = Var2.' varName3{k} '.colorMapCell;'];
% eval(ColMapEVAL);
% pSigValsEVAL = ['pSigVals = Var2.' varName3{k} '.PsigTimeAll;'];
% eval(pSigValsEVAL);
...
for k = 1:length(varName)
    ...
    varName3{k} = varName2;
    eval(['Var2.' varName2 ' = load(allMats{k});']);
    
end
varName3 = varName3';
%% plot 6 variables for each cell
saveLoc = ['C:\Users\maire\Documents\PLOTS\all_L_T_P_sigTEST_point01\'];
mkdir(saveLoc)
smoothVar = 2;
%               varName3'
kNums = [4 3 5 6 1 2]; %1:length(varName3)
timeWin = -100 :200;
signalTime = Var2.medianPval_Lall_plotVars.signalIndex;
startLookSignif = find(timeWin(signalTime)== 5);%5 mas after basline

isModulated = ones(length(kNums), length(U));
for kk = 1:length(U)
    disp(num2str(kk))
