...
for cellNum =[1:12]
    ...
    numVars = 2; 
    for iter1 = 1:numVars
        clear var1 var1LinInds var1Inds var1SummedSpikes allVar1LinInds var1AllShiftedSpikes
        hold on
        subplot(hsub1)
        ...
        var1AllShiftedSpikes = zeros(length(var1),10000);
        hold on
        for i = 1:length(var1) % i through all the go's
            if sum(U{cellNum}.R_ntk(1,:,var1(i)))>0%if there are spikes in this trial...
                ...
                lickShift = firstLicks(var1(i));%trials numbers for trial type selected above is --> var1(i)
