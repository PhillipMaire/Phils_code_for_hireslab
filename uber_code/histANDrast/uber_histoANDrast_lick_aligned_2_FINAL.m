...
for cellNum =1:length(U2)
    ...
    %%%%%%%choose one below
    alignAtZero= 'firstlick';
    %   alignAtZero= 'answerlick'
    %%%%%%%choose one above
    %%%%red dots will plot the opposite licks than the ones chosen here
    
    clf;
    plotRedDotForSecondaryLicks = 1;
    fileNameAddOn = 'raster_'
    ...
    if strcmp(alignAtZero, 'firstlick')
        alignToThisVar = firstLicksFinal';%##setvar
        otherLicksToPlotSet = answerLickTimes;%##setvar  #firstLicksFinal' #answerLickTimes
    elseif strcmp(alignAtZero, 'answerlick')
        otherLicksToPlotSet = firstLicksFinal';%##setvar
        alignToThisVar = answerLickTimes;%##setvar  #firstLicksFinal' #answerLickTimes
    end
    
    
    whichSetToUes = 3; %###setvar
    if whichSetToUes == 1
        numVars = 2;
    elseif whichSetToUes ==2
        numVars = 4;
    elseif whichSetToUes ==3
        numVars = 1;
    end
    for iter1 = 1:numVars
        ...
        elseif whichSetToUes ==3
            colorSet = {'k', 'r', 'g', 'k'};%for tghe trial type%##setvar
            %plot all trials
            if iter1 == 1
                allTrial = ones(1, length(hit));
                var1 = find(allTrial) ;
                addLength = 0;
                length1 = length(var1);
            elseif iter1 == 2
        ...
        poleOnsetTimes = poleOnsetTimesALL(var1) ;
        ...
        if sortONsecondary ==1
            ...
            poleOnsetTimes = poleOnsetTimesALL(firstLickSortInd2);
        end
        if mean(otherLicksToPlotSet == alignToThisVar)
        ...
        if sortONprimary == 1
            ...
            poleOnsetTimes = poleOnsetTimesALL(firstLickSortInd);
        end
        ...
        if plotRedDotForSecondaryLicks == 1
