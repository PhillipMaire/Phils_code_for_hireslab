...
%             if sum(U{cellNum}.R_ntk(1,:,var1(i)))>0%if there are spikes in this trial...
                ...
                lickShift = round(shiftRastBy(var1(i)));%trials numbers for trial type selected above is --> var1(i)
...
% % %         [NANI, NANJ] = find(isnan(allVar1LinInds));
        

        
        plot(J,I+addLength,'k.')
        hold on; 
% % %         plot(NANJ, NANI+addLength, 'b.')
        ...
        otherLicksToPlot = firstLicksFinal(var1)';
         plot(otherLicksToPlot+IndicesTrialStart+108, trialsToPlot,'r.')
        
        [firstlickSorted, firstLickSortInd] = sort(IndicesTrialStart);
        clear linIndTrialStart IndicesTrialStart IndicesTrialEnd
        var1SummedSpikes = nanmean(allVar1LinInds, 1);
        %note that this makes trials with no spikes all zeros so that they
        %are averaged across all 10000 points. I dont think there is a good
        %way to deal with this considering if i incuded then it would boost
        %the spike rate up artificially. i could just use the mean lick
        %time for the trial type and make the 4000 zereos around that lick
        %time and the borders be nans...
        
        
        %was the color at the end of the trials to show trial type, not
        %thats replaced with the 
...
% % % %                 normDivBy = 0;
% % % %             end
            plot(smooth(var1SummedSpikes,smoothFactor,smoothType),colorSet{iter1})
