%% PLOTS USING FILTERS
% plotsFromFilters_varFilterHEATzSCORE
%%
...
for cellStep = 1:length(U)
    ...
    %%
    mapForHeatNaned = zeros(varFiltStep, trialLength);
    for tmpk = 1:varFiltStep
        mapForHeatNaned(tmpk, 1: length(mapForHeat{tmpk})) =  mapForHeat{tmpk};
    end
    %% make z score set max and min to a certain std and remove ends after smoothing
    LimitZscore = 2.5;
    mapForHeatNaned = zscore(mapForHeatNaned);
    mapForHeatNaned(mapForHeatNaned<-LimitZscore) = -LimitZscore;
    mapForHeatNaned(mapForHeatNaned>LimitZscore) = LimitZscore;
    mapForHeatNaned = mapForHeatNaned+LimitZscore;
    for asdjfkjdskfj = 1:size(mapForHeatNaned, 1) %smooith that shit
        mapForHeatNaned(asdjfkjdskfj, :) = smooth(mapForHeatNaned(asdjfkjdskfj,:), SMOOTHfactor);
        mapForHeatNaned(asdjfkjdskfj,1:5) = nan;
        mapForHeatNaned(asdjfkjdskfj,end-4:end) = nan;
    end
    %%
