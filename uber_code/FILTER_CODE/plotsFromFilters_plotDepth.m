    ...
    for varFiltStep = 1:length(variableFilterAll)
        ...
        for k = thesetrialTypes
            ...
            if ~isempty(findEnd)
                ...
                if ~exist('setMainFilterAsBLandDontChangeSPK',   'var') ==1
                    baslineType = false;% default to minus
                end
                ...
                if setMainFilterAsBLandDontChangeSPK
                ...
                selectedRespMeanPlotTT{k} = mean(selectRespNormTT{k});
        ...
        if ~exist('plotType', 'var') || strcmp(plotType, 'response');
            for k = thesetrialTypes
                if ~(nanmean(selectedRespMeanPlotTT{k})== 0)
                    hold on
                    plotCurvesHandle = plot(selectedRespMeanPlotTT{k},U{cellStep}.details.depth, '.k');
                    ...
                    plotCurvesHandle.MarkerSize = MarkerSize;
...
ylabel(yLabelVar);
xlabel(xLabelVar);
 plotCurvesHandle.Parent.XLim = [-15 50];
  plotCurvesHandle.Parent.XScale = 'linear'
 
 grid on
% plotCurvesHandle.Parent.YLim = 


if saveON
    filename = [saveDir, filesep, saveStringAddOn, ];
