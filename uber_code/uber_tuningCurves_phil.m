...
%     
    ...
    for i=1:length(theta_sp);
        [phat, pci] = binofit(round(sum(Tsorted{i})),numel(Tsorted{i}));
    ...
    for i=1:length(FR_phase);
        [phat, pci] = binofit(round(sum(Psorted{i})),numel(Psorted{i}));
    ...
    for i=1:length(amp_sp);
        [phat, pci] = binofit(round(sum(Asorted{i})),numel(Asorted{i}));
    ...
    for i=1:length(amp_sp);
        [phat, pci] = binofit(round(sum(Ssorted{i})),numel(Ssorted{i}));
