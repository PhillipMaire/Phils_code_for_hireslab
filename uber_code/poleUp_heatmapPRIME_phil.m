...
% % % %     
    cellInfoTitle
%        cellTitleName = [' ', addToName, ' cell-', projectCellNum, ' depth-',    depth, ' ' ,mouseName, ' ' ,sessionName, ' ', cellNumString, ' ', cellCode, '.eps'];
    %%
    
    
    close all;
    
    countThresh = 1; %min touch in each bin to be considered
...
% %     XstepSize = 25; %step size for x values AUTO SET BY MATLAB
    ...
    [varspikes, preDvarspikes,postDvarspikes] = assist_varAtPoleUp(U{cellNum},window);
    ...
    for g = 1:length(comp)
        %%
        %Plot theta at touch
        fields = [  {'theta'}        {'velocity'}       {'amplitude'}    {'setpoint'}   ...
            {'phase'}               {'kappa'}      {'polePosition'}                     ];
        V.bounds = [{[-99.5:1:99.5]} {[-9750:50:9750]} {[-99.5:.5:99.5]} {[-99.5:.5:99.5]}...
            {linspace(-pi,pi,180)} {[-.95:.005:.95]}        {[2 2 2]}                            ];
        V.NumYticks = [{10} {10} {10} {10} {10} {10} {10}];
...
%             plot([0, 0],[V.(fields{d}).range(1), V.(fields{d}).range(end)],'w:') %plotting touch onset line 
        ...
        %% save it 
       if saveON == 1 
           cellTitleName = strcat(figNames{g}, cellTitleName);
             saveas(gcf,cellTitleName,'epsc')
