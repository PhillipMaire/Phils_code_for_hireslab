...
for cellStep = 1:length(cellNumsFromWrapper)
    ...
    clearvars -except   allCONTACTS   cellNumberForProject   catchcell k cellNumberForProjectString progressWin3  progressWin2 cellStep ...
        ...
        saveNameTryThisFirst dateString allCellString finalDir U U2 U3 U4 U5
    ...
    if exist([StimTrialMatsLocation,filesep, stimTrialvar, '.mat'], 'file') == 2
        ...
        for k = 1:size(allXSG,2)
            ...
            %%%%
            stimArrayNames = {'10_Hz_0' ;  'full_power_4' ; '2sec@T=1_5'  ; '100ms@T=3_5'  ; 'do_nothing_3' };
            if  isequal((testIfStim{1, k}),  '10_Hz_0')
                testIfStim{3, k} = 1;
            elseif isequal((testIfStim{1, k}),  'full_power_4')%full power
                testIfStim{3, k} = 2;
            elseif isequal((testIfStim{1, k}), '2sec@T=1_5')
                testIfStim{3, k} = 3;
            elseif isequal((testIfStim{1, k}),  '100ms@T=3_5')
                testIfStim{3, k} = 4;
            elseif isequal((testIfStim{1, k}), 'do_nothing_3')
                testIfStim{3, k} = -1;
            else
                testIfStim{3, k} = 0;
                warning('there is a stim protocol name that doesnt match I just a dumb PC i dunno what 2 dooooo')
                testIfStim{1, k}
                keyboard
            end
        end
        ...
        isStimStruct.stimArrayNames = stimArrayNames;
    ...
    for i = 1:length(useTrials);
        ...
        if isfield(contacts{useTrials(i)},'segmentInds')
            if ~isempty(contacts{useTrials(i)}.segmentInds{traj})
                ...
                firstTouchAll = round(1000*T.trials{useTrials(i)}.whiskerTrial.time{traj}(contacts{useTrials(i)}.segmentInds{traj}(1,1)...
                    :contacts{useTrials(i)}.segmentInds{traj}(1,2)));
                ...
                if size(contacts{useTrials(i)}.segmentInds{traj},1)>1
                    ...
                    for j = 2:size(contacts{useTrials(i)}.segmentInds{traj},1)
                        lateTouchAll = cat(2,lateTouchAll,round(1000*T.trials{useTrials(i)}.whiskerTrial.time{traj}...
                            (contacts{useTrials(i)}.segmentInds{traj}(j,1):contacts{useTrials(i)}.segmentInds{traj}(j,2))));
                    end
    ...
    for i =1:length(useTrials);
        spiketimes = round((T.trials{useTrials(i)}.spikesTrial.spikeTimes ./ 10)...
